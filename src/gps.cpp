...
const uint8_t initGpsM10[] = {
        ...
        0x2A,0x00,0x91,0x20,0x01,
        0x43,0x00,0x91,0x20,0x01,
        ...
        0x1E, 0,  //length payload here after
...
//        break;
    ...
    case MSG_PVT:                                // this message does not exist in ublox6 (but SOL does not exist in ublox10)
        ...
        if (( _buffer.pvt.valid & 0x07) == 0X07 ){ // if date and time are valid (bit 0 1 and 2 = HIGH validDate, ValidTime, FullyResolved)
            //printf("nbr sat : %X \n", GPS_numSat) ;
    ...
    case MSG_TIMEUTC:   // Parse time informations
        if (_buffer.timeutc.flag & 0b111) {
            gpsDate =_buffer.timeutc.year % 100;
            gpsDate <<= 8;
            gpsDate += _buffer.timeutc.month;
            gpsDate <<= 8;
            gpsDate += _buffer.timeutc.day;
            gpsDate <<= 8;
            gpsDate += 0xFF;
            gpsTime = _buffer.timeutc.hour;
            gpsTime <<= 8;
            gpsTime += _buffer.timeutc.min;
            gpsTime <<= 8;
            gpsTime += _buffer.timeutc.sec;
            gpsTime <<= 8;
            if ( prevGpsTime != gpsTime) {
                prevGpsTime = gpsTime; 
                sent2Core0(GPS_DATE , gpsDate);
